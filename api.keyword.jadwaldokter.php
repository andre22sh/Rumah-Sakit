...
switch ($type) 
{
	case 1:
		...
		if (count($pesanError)>=1){
			...
			$mySql = "INSERT INTO daftar_dokter (id_dokter,nama_dokter,alamat_dokter,no_telp,kd_spesialis,spesialis)
  ...
  case 97:
			...
			while ($myData = mysqli_fetch_array ($myQry)) {
				...
				$data.= sprintf("[\"%s\",\"%s\",\"%s\",\"%s\",\"%s\",\"%s\"],", 
								$i,$myData['id_dokter'],$myData['nama_dokter'],$myData['alamat_dokter'],$myData['no_telepon'],$myData['kd_spesialis'],$myData['spesialis']);
				$i++;
